...
export default function DragContext() {
  const [items, setItems] = useState(initialSites);
  const [bounds, setBounds] = useState({
    main: { width: 0, height: 0 },
    secondary: { width: 0, height: 0 },
  });

  // Update drop area bounds
    ...
    setItems((prevItems) =>
      prevItems.map((item) => {
        if (item.id === active.id) {
          ...
          return { ...item, x: newX, y: newY, area: newArea as string };
        }
        return item;
      })
    );
